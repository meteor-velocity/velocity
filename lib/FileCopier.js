...
_.extend(Velocity.FileCopier.prototype, {
  ...
  _onFileAdded: function (newFile) {
    if (newFile && this.options.shouldCopy(newFile)) {
      this._replaceFileInMirror(newFile, this.options.onFileAdded.bind(this, newFile));
    }
  },

  _onFileChanged: function (newFile, oldFile) {
    if (oldFile && this.options.shouldCopy(oldFile)) {
      // Remove the oldFile in case the absolutePath has changed
      this._removeFileFromMirror(oldFile);
    }
    if (newFile && this.options.shouldCopy(newFile)) {
      this._replaceFileInMirror(
        newFile,
        this.options.onFileAdded.bind(this, newFile)
      );
    }
  },
  ...
  _removeFileFromMirror: function (file, callback) {
    ...
    fs.remove(mirrorFilePath, function (error) {
      if (error) {
        DEBUG && console.log(
          '[Velocity.FileCopier] Error while removing file ' + mirrorFilePath,
          error
        );
  ...
  _replaceFileInMirror: function (file, callback) {
    var self = this;
    ...
    DEBUG && console.log('[Velocity.FileCopier] with', file.absolutePath);
    fs.copy(file.absolutePath, mirrorFilePath, function (error) {
      if (error) {
        DEBUG && console.log(
          '[Velocity.FileCopier] Error while replacing file ' + mirrorFilePath,
          error
        );
      } else if (_.isFunction(callback)) {
        callback();

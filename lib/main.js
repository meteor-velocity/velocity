...
console.log('METEOR TEST RUNNER STARTING')
...
gaze(FILE_WATCHER_MATCHING_PATTERN, {cwd: ABSOLUTE_TESTS_DIR}, Meteor.bindEnvironment(function (err, watcher) {
    ...
    var extractFilename = function (file) {
        ...
        addTestFile = function (filename, path) {
            if (filename.match(GENERIC_TEST_REGEX)) {
                var ret = MeteorTestRunnerTestFiles.insert({
    ...
    Meteor.methods({
        ...
        resetReports: function (options) {
            ...
            MeteorTestRunnerTestReports.remove(query);
        ...
        postResult: function (options) {
            ...
            var existingResult = MeteorTestRunnerTestReports.findOne(options.id);
            if (existingResult) {
                MeteorTestRunnerTestReports.update(options.id, {$set: result});
            } else {
                MeteorTestRunnerTestReports.insert(_.extend(result, {_id: options.id}));
            }
            updateAggregateReports();
        }
    });
